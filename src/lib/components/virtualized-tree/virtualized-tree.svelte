...
<script lang="ts">
	...
	import { VirtualizedTree } from '.';

	// Sample tree data
	export let virtualizedTree: VirtualizedTree<any>;
	...
	const getParentIds = (newExpanded: Set<number>, node: TreeItem<any>) => {
		...
		//Only the root node is null, so just return the root nodes oid
		if (node.parent === null) return newExpanded.add(node.id);
		//Otherwise push the current nodes oid
